...
vi.mock('../Markdown', () => ({ default: (props: { message: string }) => <div>{props.message}</div> }))
...
describe('ShareModal', () => {
  const renderModal = (override?: Partial<React.ComponentProps<typeof ShareModal>>) => {
    const qc = new QueryClient()
    const setOpen = vi.fn()
    const props = {
      messages: [
        { isBot: false, message: 'Hello', images: [], name: 'user', modelName: undefined },
        { isBot: true, message: 'Hi there', images: [], name: 'gpt-4o', modelName: 'gpt-4o' }
      ] as any,
      historyId: 'hist-1',
      open: true,
      setOpen,
  ...
  it('submits, copies link, saves webshare and closes modal', async () => {
    ...
    const submitBtn = await screen.findByRole('button', { name: /share.form.btn.save/i })
    fireEvent.click(submitBtn)

    await waitFor(() => {
      // clipboard called with generated URL without trailing slash
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith('http://localhost/share/chat-123')
      // modal closed
