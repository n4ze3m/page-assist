...
import { EraserIcon, PanelLeftCloseIcon } from "lucide-react"
...
export const DesktopSidebar: React.FC<Props> = ({
  ...
  return (
    <aside
      className={`hidden lg:flex lg:flex-col transition-all duration-300 ease-in-out border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-[#0d0d0d] ${
      ...
      <div className="flex flex-col h-full w-full">
          ...
          <div className="flex items-center gap-2">
            <Tooltip
              ...
              <button
                ...
                className="text-gray-600 hover:text-gray-800 dark:text-gray-300 dark:hover:text-gray-100 p-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors">
                <EraserIcon className="size-4" />
              </button>
            </Tooltip>
            <Tooltip title={t("common:closeSidebar")} placement="bottom">
              <button
                onClick={onClose}
                className="text-gray-600 hover:text-gray-800 dark:text-gray-300 dark:hover:text-gray-100 p-1.5 rounded-md hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors">
        ...
        <div className="flex-1 overflow-hidden">
          <div
            className={`h-full flex flex-col ${temporaryChat ? "pointer-events-none opacity-50" : ""}`}>
            <div className="sticky top-0 z-10 px-3 py-2 bg-gray-50 dark:bg-[#0d0d0d]">
            ...
            {status === "success" && orderedChatHistories.length > 0 && (
              <div className="flex flex-col gap-2 overflow-y-auto flex-1 px-3 pb-3 custom-scrollbar">
                {orderedChatHistories.map((group, groupIndex) => (
                  <div key={groupIndex}>
                    <div className="flex items-center justify-between mt-2">
                      <h3 className="px-1 text-xs font-medium text-gray-500 truncate">
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                      {group.items.map((chat, index) => (
                        <div
                          key={chat.id}
                          className={`flex py-2 px-2 items-center gap-2 relative rounded-md truncate group transition-opacity duration-300 ease-in-out border ${
                            historyId === chat.id
                          ...
                          <button
                            className="flex-1 overflow-hidden text-start truncate min-w-0"
                            onClick={async () => {
                              const db = new PageAssistDatabase()
                              const history = await db.getChatHistory(chat.id)
                              const historyDetails = await db.getHistoryInfo(
                                chat.id
                              )
                              ...
                              if (isLastUsedChatModel) {
                                const currentChatModel =
                                  historyDetails?.model_id
                                if (currentChatModel) {
                                  setSelectedModel(currentChatModel)
                                }
                              }
                              const lastUsedPrompt =
                                historyDetails?.last_used_prompt
                              if (lastUsedPrompt) {
                              ...
                              overlay={
                                <Menu>
                                  ...
                                  <Menu.Item
                                    ...
                                    onClick={() => {
                                      if (
                                        !confirm(t("deleteHistoryConfirmation"))
                                      )
                                        return
