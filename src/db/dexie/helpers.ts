...
export const removeMessageUsingHistoryId = async (history_id: string) => {
  ...
  if (chatHistory.length > 0) {
    const firstMessage = chatHistory.sort((a, b) => b.createdAt - a.createdAt)[0];
...
export const importChatHistoryV2 = async (data: any[], options: {
...
export const importPromptsV2 = async (data: Prompt[], options: {
  replaceExisting?: boolean;
  mergeData?: boolean;
} = {}) => {
  const chatDb = new PageAssistDatabase();
