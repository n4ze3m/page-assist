...
import { getIsSimpleInternetSearch, totalSearchResults, getBraveApiKey } from "@/services/features/search"
...
export const braveAPISearch = async (query: string) => {
    ...
    const docsRes = await store.similaritySearchKB(query, 3)

    const searchResult = docsRes.map((doc) => {
        return {
