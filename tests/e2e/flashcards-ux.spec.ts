...
test.describe('Flashcards workspace UX', () => {
  ...
  test('connected workspace shows tabbed UX and helpful empty states', async () => {
    ...
    // Review tab: friendly empty state when no cards are due
    const noDueHeading = page.getByText(/No cards due for review/i)
    await expect(noDueHeading).toBeVisible()
    await expect(
