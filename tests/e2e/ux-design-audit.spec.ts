...
function logIssue(uxIssues: UXIssue[], issue: UXIssue) {
  uxIssues.push(issue)
  console.log(
    `[UX ${issue.severity.toUpperCase()}] ${issue.workflow}: ${issue.issue}`
  )
}
  ...
  test.describe("1. First-Run Onboarding Experience", () => {
    test("fresh install shows clear onboarding with progress indication", async ({
      ...
      try {
        ...
        const hasWelcome = await page
          .getByText(/welcome|get started|let's connect/i)
          .isVisible()
          .catch(() => false)
        const hasStepIndicator = await page
          .getByText(/step\s*\d|1\s*of\s*\d/i)
          .isVisible()
          .catch(() => false)
        const hasProgressBar = await page
          .locator('[role="progressbar"], .progress, .stepper')
          .isVisible()
          .catch(() => false)
        ...
        if (!hasWelcome) {
          logIssue(uxIssues, {
            ...
            expected:
              "User should see a welcoming message explaining what the extension does"
          })
        }
        ...
        if (!hasStepIndicator && !hasProgressBar) {
          logIssue(uxIssues, {
            ...
            expected:
              'User should see "Step 1 of 3" or a progress bar to set expectations'
          })
        }

        // Check server URL input clarity
        const urlInput = page
          .getByLabel(/server\s*url/i)
          .or(page.getByPlaceholder(/server|url|http/i))
        ...
        if (urlInputVisible) {
          ...
          if (!placeholder || !placeholder.includes("http")) {
            logIssue(uxIssues, {
              ...
              expected:
                'Placeholder should show example like "http://localhost:8000"'
            })
          }

          // Check for help text explaining what a tldw server is
          const hasHelpText = await page
            .getByText(/tldw.*server|what.*server|where.*find/i)
            .isVisible()
            .catch(() => false)
          if (!hasHelpText) {
            logIssue(uxIssues, {
              ...
              workflow: "Onboarding",
              issue:
                'No explanation of what a "tldw server" is or where to get one',
              expected:
                "First-time users need context: link to docs, explanation of the server"
            })
          }
        }

        await page.screenshot({
          path: "test-results/ux-audit-onboarding-fresh.png"
        })
      } finally {
        await context.close()
      }
      ...
      try {
        ...
        if (await urlInput.isVisible()) {
          ...
          const hasErrorIndicator = await page
            .getByText(/invalid|error|not.*valid/i)
            .isVisible()
            .catch(() => false)
          const hasRedBorder = await urlInput
            .evaluate((el) => {
              const styles = window.getComputedStyle(el)
              return (
                styles.borderColor.includes("red") ||
                styles.borderColor.includes("rgb(239")
              )
            })
            .catch(() => false)
          ...
          if (!hasErrorIndicator && !hasRedBorder) {
            logIssue(uxIssues, {
              ...
              expected:
                "User should see red border or error message when URL format is invalid"
            })
          }
          ...
          const hasReachabilityFeedback = await page
            .getByText(/cannot.*reach|unreachable|connection.*failed|offline/i)
            .isVisible()
            .catch(() => false)
          if (!hasReachabilityFeedback) {
            logIssue(uxIssues, {
              ...
              workflow: "Onboarding",
              issue:
                "No feedback when server URL is valid format but unreachable",
              expected:
                'User should see "Cannot reach server" message with retry option'
            })
          }
          ...
          const hasSuccessFeedback = await page
            .getByText(/connected|reachable|success|ready/i)
            .isVisible()
            .catch(() => false)
          if (!hasSuccessFeedback) {
            logIssue(uxIssues, {
              ...
              expected:
                'Green checkmark or "Server connected" message for positive reinforcement'
            })
          }
        }
      } finally {
    ...
    test("auth mode selection is clear and complete", async ({ uxIssues }) => {
      ...
      try {
        ...
        const singleUserOption = await page
          .getByText(/single.*user|api.*key/i)
          .isVisible()
          .catch(() => false)
        const multiUserOption = await page
          .getByText(/multi.*user|username|password/i)
          .isVisible()
          .catch(() => false)

        if (singleUserOption || multiUserOption) {
          // Check for explanatory text about each mode
          const hasAuthExplanation = await page
            .getByText(/recommended|choose|when.*use/i)
            .isVisible()
            .catch(() => false)
          if (!hasAuthExplanation) {
            logIssue(uxIssues, {
              ...
              expected:
                'Brief description: "Single User: For personal use" vs "Multi User: For shared servers"'
            })
          }
        }
        ...
        const apiKeyInput = page
          ...
          .or(page.getByPlaceholder(/api.*key|enter.*key/i))
        if (await apiKeyInput.isVisible()) {
          ...
          if (inputType !== "password") {
            logIssue(uxIssues, {
              ...
              expected:
                "API keys should be masked by default with show/hide toggle"
            })
          }

          // Check for "where to find API key" help
          const hasKeyHelp = await page
            .getByText(/where.*find|how.*get|generate/i)
            .isVisible()
            .catch(() => false)
          if (!hasKeyHelp) {
            logIssue(uxIssues, {
              ...
              expected:
                'Link or explanation: "Find your API key in tldw server settings"'
            })
          }
        }

        await page.screenshot({
          path: "test-results/ux-audit-onboarding-auth.png"
        })
      } finally {
        await context.close()
      }
      ...
      try {
        ...
        for (const route of routes) {
          ...
          const navLinks = page.locator(
            'nav a, [role="navigation"] a, .nav-link, .menu-item'
          )
          const activeLink = navLinks.locator(
            '.active, [aria-current="page"], [data-active="true"]'
          )

          const hasActiveIndicator = (await activeLink.count()) > 0
          if (!hasActiveIndicator) {
            // Check for visual active state via class
            const anyActiveState =
              (await page
                .locator(
        ...
        const hasBreadcrumb = await page
          .getByText(/settings\s*[>\/]\s*knowledge/i)
          .isVisible()
          .catch(() => false)
        ...
        if (!hasBreadcrumb && !hasBackButton) {
          logIssue(uxIssues, {
            ...
            expected:
              'Show "Settings > Knowledge" breadcrumb or back button for context'
          })
        }
      ...
      try {
        ...
        if (focusedElement === 0) {
          logIssue(uxIssues, {
            ...
            expected:
              "Tab should cycle through focusable elements with visible focus ring"
          })
        }

        // Check for visible focus indicator on the active element
        ...
        if (!hasFocusRing) {
          logIssue(uxIssues, {
            ...
            expected:
              "All focusable elements should show a clear focus ring for keyboard users"
          })
        }
      } finally {
        await context.close()
  ...
  test.describe("3. Chat Experience", () => {
    test("empty chat state guides user action", async ({ uxIssues }) => {
      ...
      try {
        ...
        const hasEmptyStateMessage = await sidepanel
          .getByText(/start.*conversation|type.*message|ask.*question/i)
          .isVisible()
          .catch(() => false)
        const hasExamples = await sidepanel
          .getByText(/example|try asking|suggest/i)
          .isVisible()
          .catch(() => false)
        ...
        if (!hasEmptyStateMessage) {
          logIssue(uxIssues, {
            ...
            expected:
              'Show helpful text like "Start a conversation" or example prompts'
          })
        }

        // Check message input is clearly visible and accessible
        const messageInput = sidepanel
          .getByPlaceholder(/type.*message|ask|chat/i)
          .or(sidepanel.getByRole("textbox"))
        ...
        if (!inputVisible) {
          logIssue(uxIssues, {
            ...
            expected:
              "Chat input should be prominently visible at bottom of sidepanel"
          })
        }

        // Check for model selection if no model configured
        const hasModelSelector = await sidepanel
          .getByRole("combobox")
          .or(sidepanel.getByText(/select.*model|choose.*model/i))
          .isVisible()
          .catch(() => false)
        const hasNoModelWarning = await sidepanel
          .getByText(/no.*model|select.*model.*first/i)
          .isVisible()
          .catch(() => false)
        ...
        if (!hasModelSelector && !hasNoModelWarning) {
          logIssue(uxIssues, {
            ...
            expected:
              "Show current model or selector so user knows which AI they're talking to"
          })
        }

        await sidepanel.screenshot({
          path: "test-results/ux-audit-chat-empty.png"
        })
      } finally {
        await context.close()
      }
    ...
    test("sending message shows clear feedback states", async ({
      ...
      try {
        ...
        const messageInput = sidepanel
          .getByPlaceholder(/type.*message/i)
          .or(sidepanel.getByRole("textbox"))
          .first()
        ...
        if (await messageInput.isVisible()) {
          ...
          const sendButton = sidepanel
            .getByRole("button", { name: /send|submit/i })
            .or(sidepanel.locator('button[type="submit"]'))
          ...
          if (!hasSendButton) {
            logIssue(uxIssues, {
              ...
              expected:
                "Visible send button for discoverability, especially on mobile"
            })
          }
          ...
          const hasLoadingIndicator = await sidepanel
            .locator(
              '.loading, .thinking, .typing, [class*="spinner"], [class*="loading"]'
            )
            .or(sidepanel.getByText(/thinking|generating|typing/i))
            .isVisible()
            .catch(() => false)
          ...
          if (!hasLoadingIndicator) {
            logIssue(uxIssues, {
              ...
              expected:
                "Show typing indicator or spinner while generating response"
            })
          }

          // Check that user message is shown
          const userMessageVisible = await sidepanel
        ...
        await sidepanel.screenshot({
          path: "test-results/ux-audit-chat-message.png"
        })
      } finally {
        await context.close()
      }
  ...
  test.describe("4. Feature Empty States", () => {
    test("Media page shows helpful empty state", async ({ uxIssues }) => {
      ...
      try {
        ...
        // If the workspace already has media items, skip empty-state critique.
        ...
        const hasEmptyStateText = await page
          .getByText(/no.*media|no.*content|get.*started|nothing.*here/i)
          .isVisible()
          .catch(() => false)
        const hasAddButton = await page
          .getByRole("button", { name: /add|upload|import|ingest/i })
          .isVisible()
          .catch(() => false)
        const hasHelpfulGuide = await page
          .getByText(/how.*add|import.*media|get.*content/i)
          .isVisible()
          .catch(() => false)
        ...
        if (!hasEmptyStateText && !hasAddButton) {
          logIssue(uxIssues, {
            ...
            expected:
              'Show "No media yet. Import content to get started" with clear CTA'
          })
        }
    ...
    test("Notes page shows helpful empty state", async ({ uxIssues }) => {
      ...
      try {
        ...
        // If there are existing notes, we are not in an empty state.
        const hasNoteItems = await page
          .locator("button", { hasText: /note/i })
          .first()
          .isVisible()
          .catch(() => false)
        if (hasNoteItems) {
        ...
        const hasEmptyState = await page
          .getByText(/no.*notes|create.*first|get.*started/i)
          .isVisible()
          .catch(() => false)
        ...
        if (!hasEmptyState && !hasCreateButton) {
          logIssue(uxIssues, {
            ...
            expected:
              'Show welcoming empty state with "Create your first note" CTA'
          })
        }
    ...
    test("Flashcards page shows helpful empty state", async ({ uxIssues }) => {
      ...
      try {
        ...
        const hasEmptyState = await page
          .getByText(/no.*flashcard|no.*deck|create.*first|start.*learning/i)
          .isVisible()
          .catch(() => false)
        const hasCreateButton = await page
          .getByRole("button", { name: /create|new|add|import/i })
          .isVisible()
          .catch(() => false)
        ...
        if (!hasEmptyState) {
          logIssue(uxIssues, {
            ...
            expected:
              'Show benefits: "Create flashcards to retain what you learn"'
          })
        }

        await page.screenshot({
          path: "test-results/ux-audit-flashcards-empty.png"
        })
      } finally {
        await context.close()
      }
  ...
  test.describe("5. Error States & Recovery", () => {
    test("server disconnection shows clear error with retry option", async ({
      ...
      try {
        ...
        const hasErrorIndicator = await page
          .getByText(
            /cannot.*connect|connection.*failed|offline|unreachable|error/i
          )
          .isVisible()
          .catch(() => false)
        const hasRetryButton = await page
          ...
          .isVisible()
          .catch(() => false)
        const hasSettingsLink = await page
          .getByRole("link", { name: /settings|configure/i })
          .or(page.getByRole("button", { name: /settings|configure/i }))
          .isVisible()
          .catch(() => false)
        ...
        if (!hasErrorIndicator) {
          logIssue(uxIssues, {
            ...
            expected:
              'Clear error: "Cannot connect to server. Check your settings or retry."'
          })
        }
        ...
        await page.screenshot({
          path: "test-results/ux-audit-connection-error.png"
        })
      } finally {
        await context.close()
      }
    ...
    test("invalid API key shows helpful error message", async ({
      ...
      try {
        ...
        if (await messageInput.isVisible()) {
          ...
          const hasAuthError = await sidepanel
            .getByText(/invalid.*key|unauthorized|authentication.*failed|401/i)
            .isVisible()
            .catch(() => false)
          const hasFixLink = await sidepanel
            .getByRole("button", { name: /update.*key|fix|settings/i })
            .or(sidepanel.getByRole("link", { name: /settings/i }))
            .isVisible()
            .catch(() => false)
          ...
          if (!hasAuthError) {
            logIssue(uxIssues, {
              ...
              expected:
                'Clear message: "Authentication failed. Please check your API key."'
            })
          }
        ...
        await sidepanel.screenshot({
          path: "test-results/ux-audit-auth-error.png"
        })
      } finally {
        await context.close()
      }
  ...
  test.describe("6. Settings Configuration UX", () => {
    test("settings are organized logically with clear labels", async ({
      ...
      try {
        ...
        const hasCategories =
          (await page.locator('h2, h3, [role="heading"]').count()) > 0
        if (!hasCategories) {
          logIssue(uxIssues, {
            severity: "minor",
            workflow: "Settings",
            ...
            expected:
              'Group settings under clear headings: "Connection", "Appearance", etc.'
          })
        }
      ...
      try {
        ...
        if (hasSaveButton) {
          ...
          const hasSuccessIndicator = await page
            .locator(
              '.toast, .notification, [role="alert"], [class*="success"]'
            )
            .isVisible()
            .catch(() => false)
          ...
          if (!hasSuccessToast && !hasSuccessIndicator) {
            logIssue(uxIssues, {
              ...
              expected:
                'Toast notification or inline message confirming "Settings saved"'
            })
          }
        } else {
          // Check for auto-save indication
          const hasAutoSave = await page
            .getByText(/auto.*save|changes.*saved.*automatic/i)
            .isVisible()
            .catch(() => false)
          if (!hasAutoSave) {
            logIssue(uxIssues, {
              ...
              workflow: "Settings",
              issue:
                "Unclear whether settings auto-save or require manual save",
              expected:
                'Either show save button or indicate "Changes are saved automatically"'
            })
          }
        }
      } finally {
  ...
  test.describe("7. Sidepanel Quick Actions", () => {
    test("sidepanel actions are discoverable", async ({ uxIssues }) => {
      ...
      try {
        ...
        const ingestButton = sidepanel.getByRole("button", {
          name: /ingest|add|import/i
        })
        const settingsButton = sidepanel
          .getByRole("button", { name: /settings|gear|cog/i })
          .or(sidepanel.locator('[aria-label*="settings" i]'))
        const newChatButton = sidepanel.getByRole("button", {
          name: /new.*chat|clear|reset/i
        })
        ...
        const iconButtons = sidepanel
          .locator('button:not(:has-text("."))')
          .or(sidepanel.locator('[role="button"]'))
        ...
        for (let i = 0; i < Math.min(iconButtonCount, 3); i++) {
          ...
          if (!title && !ariaLabel && !innerText.trim()) {
            logIssue(uxIssues, {
              ...
              expected:
                "All icon buttons should have title or aria-label for clarity"
            })
            break
          }
        }
        ...
        await sidepanel.screenshot({
          path: "test-results/ux-audit-sidepanel.png"
        })
      } finally {
        await context.close()
      }
