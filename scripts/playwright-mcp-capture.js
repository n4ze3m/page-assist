...
async function stopServer(child, baseUrl) {
  try {
    await new Promise((resolve) => {
      const req = http.get(new URL('/killkillkill', baseUrl), () => {
        resolve()
      })
...
async function main() {
  ...
  const optionsHtml = path.join(buildDir, 'options.html')
  const sidepanelHtml = path.join(buildDir, 'sidepanel.html')
  for (const filePath of [optionsHtml, sidepanelHtml]) {
  ...
  try {
    ...
    const server = spawn('npx', serverArgs, { stdio: ['ignore', 'pipe', 'pipe'] })
    ...
    await writeJson(path.join(artifactsDir, 'tools.json'), tools)
    const availableToolNames = tools.map((tool) => tool.name)

    const pages = [
      {
        name: 'options',
        url: new URL('/options.html', staticServer.url),
        waitText: 'Welcome — Let’s get you connected'
      },
      {
        name: 'sidepanel',
        url: new URL('/sidepanel.html', staticServer.url),
        waitText: 'Waiting for your tldw server'
      }
    ]

    const summaries = []

    for (const { name, url, waitText } of pages) {
      const gotoResult = await client.callTool({
        name: 'browser_navigate',
        arguments: { url: url.toString() }
      })
      if (gotoResult.isError) {
        summaries.push({ name, url: url.toString(), error: gotoResult })
        continue
      }
      if (waitText) {
        await client.callTool({
          name: 'browser_wait_for',
          arguments: { text: waitText }
        }).catch(async () => {
          await client.callTool({
            name: 'browser_wait_for',
            arguments: { time: 3 }
      ...
      const innerHtml = await client.callTool({
        name: 'browser_evaluate',
        arguments: { function: '() => document.body.innerHTML' }
      })

      const snapshotPath = path.join(artifactsDir, `${name}-snapshot.json`)
      await writeJson(snapshotPath, snapshot)
      const consolePath = path.join(artifactsDir, `${name}-console.json`)
      await writeJson(consolePath, consoleMessages)
      const innerHtmlPath = path.join(artifactsDir, `${name}-dom.html.json`)
      await writeJson(innerHtmlPath, innerHtml)
      ...
      summaries.push({
        ...
        consolePath,
        innerHtmlPath,
        screenshot: screenshot.content?.find((entry) => entry.type === 'text')?.text ?? ''
      })
    }
    ...
    await writeJson(path.join(artifactsDir, 'summary.json'), summaries)
    console.log(JSON.stringify({ baseUrl, artifactsDir, availableTools: availableToolNames, summaries }, null, 2))
  } finally {
